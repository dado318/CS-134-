...
void ofApp::setup(){
    ...
    mars.setRotation(0, 180, 0.f, 0.f, 1.f);
    ...
    // setting up an octree with 5 levels
    numLevel = 4;
    octree.create(mars.getMesh(0), numLevel);
}
...
void ofApp::mousePressed(int x, int y, int button) {
    ...
    // getting the time it takes in miliseconds to select data
    //
    ...
    if (octree.intersect(ray, octree.root, hitNode)) {
        ...
        for (int i = 0; i < count; i++) {
            ofVec3f vert = mesh.getVertex(hitNode.points[i]);
    ...
    cout << "Selecting data took " << endTime - startTime << " miliseconds." << endl;
}
